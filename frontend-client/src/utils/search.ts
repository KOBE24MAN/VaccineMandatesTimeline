...
import { mapToIndex } from "../hooks/useEvents";
...
export function searchTimelineEvents(matches: readonly Mandate[], allRecords: readonly Mandate[]): EventIndex[] {
  ...
  const embeddedIds = new Set(matches.map(record => record.booster_id).filter(Boolean));
  return matches.filter(record => !embeddedIds.has(record.id)).map(record => {
